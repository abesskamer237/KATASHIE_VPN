...
[ -f "$UI" ] && k_header "KATASHIE VPN • XRAY" || clear
...
curl -fsSL --connect-timeout 15 --max-time 120 https://acme-install.netlify.app/acme.sh -o /root/.acme.sh/acme.sh
...
if [[ ! -s /etc/xray/xray.crt || ! -s /etc/xray/xray.key ]]; then
    err "Certificat TLS introuvable après émission pour $domain."
fi
