...
[ -f "$UI" ] && k_header "KATASHIE VPN • SSHWS" || clear
...
wget -q --timeout=30 --tries=3 -O /etc/nginx/nginx.conf "${SERVER_HOST}/module/nginx.conf"
rm -f /etc/nginx/conf.d/default.conf

# Le certificat TLS doit exister avant le démarrage de Nginx.
if [[ ! -s /etc/xray/xray.crt || ! -s /etc/xray/xray.key ]]; then
echo "[ERROR] Certificat Xray manquant : /etc/xray/xray.crt ou /etc/xray/xray.key."
return 1
fi

domain=$(cat /etc/xray/domain 2>/dev/null || true)
if [[ -z "$domain" ]]; then
echo "[ERROR] Domaine KATASHIE introuvable dans /etc/xray/domain."
return 1
fi

sed -i "s/server_name \\*\\.xxxxxx;/server_name *.$domain;/g" /etc/nginx/nginx.conf
sed -i "s/server_name xxxxxx;/server_name $domain;/g" /etc/nginx/nginx.conf
sed -i "s#https://xxxxxx:86/#https://$domain:86/#g" /etc/nginx/nginx.conf

if ! nginx -t; then
echo "[ERROR] Configuration Nginx invalide."
return 1
fi
...
wget -q --timeout=30 --tries=3 -O /home/vps/public_html/index.html "${SERVER_HOST}/module/index"
chown -R www-data:www-data /home/vps/public_html
}
install_badvpn() {
wget -q --timeout=30 --tries=3 -O /usr/bin/badvpn-udpgw "${SERVER_HOST}/module/newudpgw"
chmod +x /usr/bin/badvpn-udpgw
wget -q --timeout=30 --tries=3 -O /etc/systemd/system/badvpn@.service "${SERVER_HOST}/module/badvpn@.service"
systemctl daemon-reload
...
wget -q --timeout=30 --tries=3 -O "stunnel-${STUNNEL_VERSION}.tar.gz" "$STUNNEL_URL"
...
-subj "/C=MY/ST=Centre/L=Yaounde/O=katashievpn/OU=stunnel/CN=$(hostname -f)/emailAddress=admin@localhost" \
